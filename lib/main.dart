...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final Map<String, int> _outputTensorIndices = {};
  ...
  Future<void> _initTflite() async {
    try {
      ...
      // Find logits output tensor index and build name-to-index map for outputs
      for (int i = 0; i < outputTensors.length; i++) {
        _outputTensorIndices[outputTensors[i].name] = i;
  ...
  Future<void> _runInference(CameraImage image) async {
    try {
      ...
      for (int i = 0; i < inputTensors.length; i++) {
        ...
        if (tensor.name == 'serving_default_image:0') continue;

        final outputIndex = _outputTensorIndices[tensor.name];
        if (outputIndex != null) {
          _modelState[i] = _outputBuffers[outputIndex]!;
        }
      }
